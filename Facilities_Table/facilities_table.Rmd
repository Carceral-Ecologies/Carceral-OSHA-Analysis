...
caprisons = do.call(rbind, by(caprisons, caprisons$estab_name, FUN = function(estab){
  ...
  # Format to two decimals
  caprisons$case_days_open_mean = as.numeric(formatC(caprisons$case_days_open_mean, digits = 2, format = "f"))
...
```{r}
# change decimal on average number of days open to round up the nearest day
# think about changing day to weeks or months for easier interpretation
# change union present to yes or no. and probably only keep this one for the table. 
# add the color bars to the new variables. 
# it would be nice to be able to add the activity nrs for people to search on the osha page.
# I need to create a count by activity nr. BEcause that gives us the total number of inspections. then we have the total nr of violations. That should help with that issue. 
