...
```{r}
violation_occurances <- table(cleaned_data$viol_type)
...
```{r}
number_instances <- cleaned_data$nr_instances
...
Our average number of instances is 5.49. The highest amount of instances is 1,115. The lowest amount of instances would be 0 as indicated by the NA output. 
...
```{r}
table(cleaned_data$noviol, cleaned_data$nr_instances)
```
...
```{r}
number_exposed <- cleaned_data$nr_exposed
...
how many were in inspection and not violation. 

table 
  number exposed and noviol. shows NA's by inspection only. of these 0's, 200 is cuz no violation. other is not reported. 
...
Which prisons had that violation and what violation associated with it. Haz_sub. helps for blog post. Identify those prisons and violation occured. 

think ab what to highlight. Descriptive table. show mean, median or mode? choose which one. 
...
```{r}
initial_penalty_column <- cleaned_data$initial_penalty
...
Ask about graphs! check quartiles and qunitiles. Showing that. Moderate, high end penalties. Check meaning of penalty. 
...
```{r}
owner_type_plot <- ggplot(owner_type_frame, aes(x=owner_type_column, y=Freq)) +geom_bar(stat = 'identity', color = 'red', fill = 'blue') + ggtitle('Frequency of the Owner Types') + xlab("Owner Types") + ylab("Frequency") + theme(plot.title = element_text(hjust = 0.5))
owner_type_plot
```
